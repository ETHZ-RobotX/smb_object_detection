...
| input_camera_name            | /visual_inertial_sensor/cam0/slow                                   | Input camera name. The exact topics needed are inferred based on the camera name. By default refers to a temporally downsampled image stream.|
...
## Launch Using Rosbag
The launch folder contains a launch file for testing the pipline based on a provided rosbag. Apart from launching the detection node and playing the rosbag, it also launches an RViz window with useful visualisation of relevant topics. Currently the existing rosbag has slightly different outputs than on the newest setup of the camera. To compensate for this, the rosbag launch file contains a topic dropper node to create the `/visual_inertial_sensor/cam0/slow` topics.
